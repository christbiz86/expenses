...
{$transfer = 0}
{foreach $list as $sql}
    {if $sql->payment=='Tunai'}
        {$tunai = $tunai + $sql->jumlah|dec}
    {else}
        {$transfer = $transfer + $sql->jumlah|dec}
    {/if}
{/foreach}
...
<table style="margin: 0 45px;width: 800px;text-align: center" border="1">
    ...
    <tr>
        ...
        <td colspan=6>{getTotalPd($row->request_id)}</td>
        <td></td>
<!--        <td rowspan=2>Dibuat</td>
    ...
    <tr>
        <td>Transfer</td>
        <td colspan=2>Rp. {$transfer|number_format:0:",":"."}</td>
        <td rowspan=2 colspan="2">{getBankData($row->bank_id)}</td>
        <td rowspan=2>{$row->no_cheque}</td>
        <td rowspan=2>{showDate($row->jatuh_tempo)}</td>
        <td rowspan="2">-</td>
    </tr>
    <tr>
        <td rowspan="4">Tunai</td>
        <td colspan=2 rowspan="4">Rp. {$tunai|number_format:0:",":"."}</td>
    </tr>
    <tr>
